...
*/
    ...
    int intBlockCount = 0;
    try {intBlockCount = Integer.parseInt(blockCount, 16);}catch (NumberFormatException e) {}

    int intSplitFileSize = -1;
    try {intSplitFileSize = Integer.parseInt(splitFileSize, 16);}catch (NumberFormatException e) {}

    int intSplitFileBlocksize = -1;
    try {intSplitFileBlocksize = Integer.parseInt(splitFileBlocksize, 16);}catch (NumberFormatException e) {}
    ...
    try {splitFileSize = Long.parseLong(SettingsFun.getValue(target.getPath(), "SplitFile.Size"), 16);}catch (NumberFormatException e) {}

    {
        ...
        catch (Exception e){System.out.println("exeption in getfecsplit " +e.toString());}
        }
    }
    ...
    for (int segmentCnt = 0; segmentCnt < segmentHeaders.size(); segmentCnt++){
        ...
        // and mark them succeeded. They are
        // not requested again
        ...
        System.out.println(new StringBuffer().append("Found ").append(availableChunks)
                                     .append(" chunks for segment ").append(segmentCnt).toString());
        ...
        for (i = 0; i < checkBlockCount; i++){
            ...
            else { //also request all the checks.
                if (requestList.length() > 0) requestList += ",";
    ...
    if (getKey(key, size, tempFile, htl)) {
        ...
        if (tempFile.length() <= 65536 && doRedirect) { // File may be a redirect
        ...
        if (isRedirect) { // File is a redirect
